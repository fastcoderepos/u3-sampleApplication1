...
import com.fastcode.sampleapplication14.domain.core.timeofftype.*;
...
import com.fastcode.sampleapplication14.domain.core.timeofftype.QTimeofftype;
...
public class TimeofftypeAppServiceTest {
	...
	public void findTimeofftypeById_IdIsNotNullAndIdExists_ReturnTimeofftype() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.timeofftypeToFindTimeofftypeByIdOutput(timeofftype));
	}
    ...
    public void createTimeofftype_TimeofftypeIsNotNullAndTimeofftypeDoesNotExist_StoreTimeofftype() { 
        ...
        Mockito.when(_mapper.createTimeofftypeInputToTimeofftype(any(CreateTimeofftypeInput.class))).thenReturn(timeofftypeEntity); 
        Mockito.when(_timeofftypeRepository.save(any(Timeofftype.class))).thenReturn(timeofftypeEntity);

	   	Assertions.assertThat(_appService.create(timeofftypeInput)).isEqualTo(_mapper.timeofftypeToCreateTimeofftypeOutput(timeofftypeEntity));
	...
	public void updateTimeofftype_TimeofftypeIdIsNotNullAndIdExists_ReturnUpdatedTimeofftype() {
		...
		Mockito.when(_mapper.updateTimeofftypeInputToTimeofftype(any(UpdateTimeofftypeInput.class))).thenReturn(timeofftypeEntity);
		Mockito.when(_timeofftypeRepository.save(any(Timeofftype.class))).thenReturn(timeofftypeEntity);
		Assertions.assertThat(_appService.update(ID,timeofftype)).isEqualTo(_mapper.timeofftypeToUpdateTimeofftypeOutput(timeofftypeEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QTimeofftype.class), any(HashMap.class), any(HashMap.class));
