...
import com.fastcode.sampleapplication14.domain.core.timesheet.*;
...
import com.fastcode.sampleapplication14.domain.core.timesheet.QTimesheet;
...
import com.fastcode.sampleapplication14.domain.core.timesheetstatus.ITimesheetstatusRepository;
...
public class TimesheetAppServiceTest {
	...
	public void findTimesheetById_IdIsNotNullAndIdExists_ReturnTimesheet() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.timesheetToFindTimesheetByIdOutput(timesheet));
	}
    ...
    public void createTimesheet_TimesheetIsNotNullAndTimesheetDoesNotExist_StoreTimesheet() { 
        ...
        Mockito.when(_mapper.createTimesheetInputToTimesheet(any(CreateTimesheetInput.class))).thenReturn(timesheetEntity); 
        Mockito.when(_timesheetRepository.save(any(Timesheet.class))).thenReturn(timesheetEntity);

	   	Assertions.assertThat(_appService.create(timesheetInput)).isEqualTo(_mapper.timesheetToCreateTimesheetOutput(timesheetEntity));
	...
	public void updateTimesheet_TimesheetIdIsNotNullAndIdExists_ReturnUpdatedTimesheet() {
		...
		Mockito.when(_mapper.updateTimesheetInputToTimesheet(any(UpdateTimesheetInput.class))).thenReturn(timesheetEntity);
		Mockito.when(_timesheetRepository.save(any(Timesheet.class))).thenReturn(timesheetEntity);
		Assertions.assertThat(_appService.update(ID,timesheet)).isEqualTo(_mapper.timesheetToUpdateTimesheetOutput(timesheetEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QTimesheet.class), any(HashMap.class), any(HashMap.class));
	...
	public void GetTimesheetstatus_IfTimesheetIdAndTimesheetstatusIdIsNotNullAndTimesheetExists_ReturnTimesheetstatus() {
		...
		Assertions.assertThat(_appService.getTimesheetstatus(ID)).isEqualTo(_mapper.timesheetstatusToGetTimesheetstatusOutput(timesheetstatusEntity, timesheet));
	}
	...
	public void GetUsers_IfTimesheetIdAndUsersIdIsNotNullAndTimesheetExists_ReturnUsers() {
		...
		Assertions.assertThat(_appService.getUsers(ID)).isEqualTo(_mapper.usersToGetUsersOutput(usersEntity, timesheet));
	}
