...
import com.fastcode.sampleapplication14.domain.core.customer.*;
...
import com.fastcode.sampleapplication14.domain.core.customer.QCustomer;
...
public class CustomerAppServiceTest {
    ...
    public void createCustomer_CustomerIsNotNullAndCustomerDoesNotExist_StoreCustomer() { 
        ...
        Mockito.when(_customerRepository.save(any(Customer.class))).thenReturn(customerEntity);
	   	...
	   	Assertions.assertThat(_appService.create(customerInput)).isEqualTo(_mapper.customerToCreateCustomerOutput(customerEntity));
	...
	public void updateCustomer_CustomerIdIsNotNullAndIdExists_ReturnUpdatedCustomer() {
		...
		Mockito.when(_customerRepository.save(any(Customer.class))).thenReturn(customerEntity);
		Assertions.assertThat(_appService.update(ID,customer)).isEqualTo(_mapper.customerToUpdateCustomerOutput(customerEntity));
	}
	...
	public void search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QCustomer.class), any(HashMap.class), any(HashMap.class));
