package com.fastcode.sampleapplication14.application.core.authorization.userspermission;

import lombok.NonNull;
import lombok.RequiredArgsConstructor;
import com.fastcode.sampleapplication14.application.core.authorization.userspermission.dto.*;
import com.fastcode.sampleapplication14.domain.core.authorization.userspermission.IUserspermissionRepository;
import com.fastcode.sampleapplication14.domain.core.authorization.userspermission.QUserspermission;
import com.fastcode.sampleapplication14.domain.core.authorization.userspermission.Userspermission;
import com.fastcode.sampleapplication14.domain.core.authorization.userspermission.UserspermissionId;
import com.fastcode.sampleapplication14.domain.core.authorization.permission.IPermissionRepository;
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
		    ...
		    if(details.getKey().replace("%20","").trim().equals("users")) {
				if(details.getValue().getOperator().equals("contains") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
				   ...
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(userspermission.users.id.between(Long.valueOf(details.getValue().getStartingValue()), Long.valueOf(details.getValue().getEndingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getStartingValue())) {
