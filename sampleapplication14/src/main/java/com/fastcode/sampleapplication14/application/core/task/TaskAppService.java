package com.fastcode.sampleapplication14.application.core.task;

import lombok.NonNull;
import lombok.RequiredArgsConstructor;
import com.fastcode.sampleapplication14.application.core.task.dto.*;
import com.fastcode.sampleapplication14.domain.core.task.ITaskRepository;
import com.fastcode.sampleapplication14.domain.core.task.QTask;
...
public class TaskAppService implements ITaskAppService {
	...
	protected BooleanBuilder searchKeyValuePair(QTask task, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
		    ...
		    if(details.getKey().replace("%20","").trim().equals("project")) {
				if(details.getValue().getOperator().equals("contains") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
				   ...
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue())) {
                	   builder.and(task.project.id.between(Long.valueOf(details.getValue().getStartingValue()), Long.valueOf(details.getValue().getEndingValue())));
                   } else if(StringUtils.isNumeric(details.getValue().getStartingValue())) {
